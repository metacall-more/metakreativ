...
export default function Testimonials() {
  ...
  return (
    <section className="mx-auto my-[110px] max-w-(--container-max) overflow-hidden rounded-[20px] border border-brand-gray-300 p-[60px]">
      <div className="flex justify-between text-sm tracking-[1.4px] text-brand-gray-600 uppercase">
      ...
      <h2
        ref={headingReveal}
        className="reveal my-5 mb-10 text-center font-caps text-[clamp(60px,8.3vw,160px)] normal-case"
      >
      ...
      <div ref={deckReveal} className="reveal reveal-delay relative min-h-[620px]">
        {Array.from({ length: CARD_COUNT }).map((_, i) => (
          <article
            key={i}
            ref={setCardRef(i)}
            className="testimonial-card absolute top-1/2 left-1/2 flex min-h-[560px] w-[430px] flex-col justify-between gap-10 rounded-[20px] bg-white p-10"
          >
            ...
            <p className="m-0 mt-5 w-full text-[95px] leading-none text-brand-ink">&ldquo;</p>

            <div className="flex flex-col gap-7 text-brand-ink-2">
              <h4 className="m-0 font-display text-[17px] font-medium uppercase">{TESTIMONIAL.heading}</h4>
              <p className="m-0 text-base leading-[1.9]">{TESTIMONIAL.quote}</p>
            </div>
          </article>
        ))}
        ...
        <button
          onClick={prev}
          className="absolute top-1/2 left-0 z-[3] h-9 w-9 -translate-y-1/2"
        ...
        <button
          onClick={next}
          className="absolute top-1/2 right-0 z-[3] h-9 w-9 -translate-y-1/2"
