...
import { useCountUp, useInViewReplay } from '../../hooks/useCountUp';
...
function StatCard({ target, suffix, label, description, active }) {
  const value = useCountUp(target, active);

  return (
    <div className="flex min-h-[320px] flex-col border-b border-brand-gray-300 p-8 sm:min-h-[400px] sm:p-11 lg:min-h-[450px] lg:border-r lg:last:border-r-0">
      <div className="mb-6">
      ...
      <p className="m-0 mt-auto font-body text-base leading-[1.65] text-[#4f5153]">{description}</p>
    </div>
  );
}
...
export default function AboutImpact() {
  ...
  const inView = useInViewReplay(sectionRef);

  return (
    <section
      ref={sectionRef}
      className="mx-auto max-w-[calc(var(--container-max)+80px)] px-5 py-16 md:px-8 md:py-20 lg:px-10 lg:py-24"
    >
